...
impl Solution {
    ...
    fn max_profit_1(prices: Vec<i32>) -> i32 {
        if prices.len() < 2 {
            return 0;
        }
        let n = prices.len();
        let mut buy1 = (-prices[0]).max(-prices[1]);
        let mut sell1 = 0.max(prices[1] - prices[0]);
        let mut sell2 = 0;
        for i in 2..n {
            let tmp = sell1;
            sell1 = tmp.max(buy1 + prices[i]);
            buy1 = buy1.max(sell2 - prices[i]);
            sell2 = tmp;
        }
        sell1
    }
...
fn test() {
    ...
    let prices: Vec<i32> = vec![1, 2, 3, 0, 2];
    let res = 3;
    assert_eq!(Solution::max_profit_1(prices), res);
}